...
- **开机自启**：勾选「开机自启」，程序自动写入 Windows 启动项（`HKCU Run`），登录即自动运行；取消勾选即可移除。
- **开机自动恢复**：勾选「开机后自动恢复上次软件」，登录延迟几秒后自动把保存的列表全部拉起，**开机即就绪，什么都不用点**。
...
5. **全自动（可选）**：勾选「开机自启」+「开机后自动恢复上次软件」，从此每天开机自动恢复，连 AppStarter 都不用打开。
